...
namespace SSoT.me.AirtableToDotNetLib
{
    ...
    public static class AirtableExtensions
    {
        ...
        {
            // Clone the original XmlDocument to keep all existing data except for modifications to attachment fields
            XmlDocument updatedXmlDoc = (XmlDocument)originalXmlDoc.CloneNode(true);

            // List to hold nodes to be replaced
            List<XmlNode> nodesToReplace = new List<XmlNode>();

            // Traverse each element in the cloned document to identify nodes to be replaced
            var attachmentNodes = updatedXmlDoc.SelectNodes("//*[url]"); // Select nodes that contain a 'url' child
            foreach (XmlNode node in attachmentNodes)
            {
                // Ensure the 'url' node is a direct child of the current node
                foreach (XmlNode childNode in node.ChildNodes)
                {
                    if (childNode.Name == "url")
                    {
                        nodesToReplace.Add(node);
                        break; // Found the url node, mark this node for replacement
                    }
                }
            }

            // Replace each identified node with a new node containing only the URL text
            foreach (XmlNode node in nodesToReplace)
            {
                XmlNode urlNode = node.SelectSingleNode("url");
                XmlNode newNode = updatedXmlDoc.CreateElement(node.Name);
